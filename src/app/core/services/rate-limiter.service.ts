...
/** Client-side rate limiter — enforces per-action write limits via in-memory buckets. Resets on page reload. */
@Injectable({
	providedIn: 'root',
})
export class RateLimiterService {
	...
	canPerform(action: string): boolean {
		...
		const windowExpired =
			config.windowMs > 0 && now - bucket.windowStart > config.windowMs;
